  ...
  return (
    <>
      <div className="scrollbar overflow-x-auto">
        <table className="w-full table-auto border-x border-b border-foreground backdrop-blur-sm">
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <tr key={headerGroup.id}>
                {headerGroup.headers.map((header) => {
                  ...
                  return (
                    <th key={header.id} className="py-2 pl-6 text-secondary">
                      <button
          ...
          <tbody className="text-sm">
            {table.getRowModel().rows.map((row) => {
              return (
                <tr
                  key={row.id}
                  className={`text-primaryThin cursor-pointer border-t border-foreground backdrop-blur-sm *:py-4 *:pl-6 hover:text-primary ${onRowClick ? 'cursor-pointer' : ''}`}
                  onClick={
